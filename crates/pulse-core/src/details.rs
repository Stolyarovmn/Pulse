...
pub struct ProcessDetails {
    ...
    pub restricted: bool,
    /// Обход дескрипторов оборван бюджетом.
    ///
    /// Процесс может держать сотни тысяч дескрипторов, а разыменование каждого
    /// — это отдельный syscall. Агент обязан иметь потолок работы, но тогда
    /// `fd_total`, `files` и `ports` описывают лишь просмотренную часть.
