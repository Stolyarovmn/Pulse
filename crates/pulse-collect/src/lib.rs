...
/// Размер страницы памяти ядра.
///
/// Хардкод 4096 занижал память в шестнадцать раз на ядрах с 64 КиБ
/// страницами — обычная сборка для aarch64 и ppc64le (PULSE-015). Значение
/// берётся из `auxv`, который ядро кладёт процессу при старте: это ровно то
/// же число, что вернул бы `sysconf(_SC_PAGESIZE)`, но без зависимости от
/// libc, которой у статического бинарника нет.
#[must_use]
