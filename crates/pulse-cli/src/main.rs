...
fn check(config: &PulseConfig, demo: bool) -> Result<(), Box<dyn Error>> {
    ...
    // Усечение по бюджету — не отказ: на большой ноде дефолтный `max_cgroups`
    // законно не покрывает всё дерево. Молчать нельзя (часть хоста не
    // наблюдается), но и падать нельзя: оператор ничего не сломал.
    if snapshot.agent.collector_truncations > 0 {
        println!(
            "warn: сбор неполон: усечений по бюджету {} - поднимите general.max_cgroups (см. timeline)",
