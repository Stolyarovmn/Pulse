...
- Inspector не является top-level screen: Inspector — contextual session;
  Search и палитра (`:` — команды места, `?`/`F1` — справка, все команды всех
  мест) — overlays с полным владением вводом. Команда палитры исполняется тем
  же `App::dispatch`, что и её клавиша (`crate::palette`).
...
Правила шага вниз (`Enter`, список `INSIDE`, источник — `investigate::drill_candidates`):
...
`INSIDE` (`investigate::descend`) показывает каждый логический объект внутри
отдельной строкой: чужие объекты свёрнуты (процессы — в свой сервис), части
самого объекта — поштучно, у unit/контейнера/pod спуск начинается сразу с их
cgroup. Порядок — состояние, затем CPU, затем память. Свёрнутая строка
`197 × cgroup` здесь запрещена: `Enter` по ней открывал бы произвольный объект.
Предпросмотры Overview/Entities используют `drill_steps` со свёрткой `N × kind`.

`LEADS` (`crate::leads`) — зацепки: открытая проблема внутри, ненормальное
состояние, доля CPU или памяти ≥ 50 % среди ≥ 2 детей, перезапуск/OOM, самый
активный сосед по диску того же вида. Цель — логический объект (у cgroup с
владельцем — владелец); одна цель — одна зацепка, сильнейшая. Проблемы самого
объекта и его двойника (`leads::twins`) — в `CASE`, не в зацепках.

`AROUND` (`investigate::side_steps`) — боковые переходы: родитель не глубже
текущего объекта (`owner` только для unit/контейнера/pod, иначе `parent`) и
соседи по общему диску с измеренным потоком > 0, кроме предков и потомков
(`investigate::is_lineage`). Переход по ним начинает **новое** расследование
(`InspectorSession::restart`): смежный объект не лежит глубже, и склеивать
«спустился» и «прыгнул» в один путь значит врать о причинной связи.

`Tab` ходит по кругу `INSIDE → LEADS → AROUND`, пропуская пустые списки.
Renderer сохраняет ключи показанных строк активного списка
(`InspectorSession::shown`): `Enter` открывает показанное, а курсор при
пересортировке остаётся на том же объекте.

Свёртка однотипных в `drill_steps`: сначала собираются уникальные объекты,
потом сворачиваются. Обратный порядок давал неверный счёт - свёрнутая строка
хранит ключ лишь первого объекта, и на живом хосте 14 процессов показывались
как «5 × process». Счётная форма `N × kind`, а не суффикс `s`: вид приходит из
реестра и не обязан быть склоняемым словом.
