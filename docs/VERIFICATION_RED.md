...
Дальнейшее снижение до 20 % интервала требует решений, меняющих контракт:
кэш `/proc/<pid>/cgroup` (членство в cgroup видно с опозданием), ревизия
`oom_score`/`io` на процесс, кэш лимитов cgroup (`cpu.max`, `memory.max`,
`pids.max`). Finding остаётся открытым.
