...
| PULSE-080 | `proc_walk_work_is_bounded_by_max_processes` + `cgroup_child_listing_is_bounded_by_budget` | pulse-collect | обход материализовал весь каталог и применял бюджет уже после: 20 000 записей `/proc` при лимите 64 | FIXED в PR-07c. Уточнение после PULSE-088: проходов теперь два и ограничены они разным. **Детальный** сбор по-прежнему ограничен `max_processes` — это проверяется числом сущностей в графе и числом чтений (`read ≤ budget × 12`). **Подсчёт процессов хоста** при переполнении бюджета делает отдельный проход по именам записей до `FULL_SCAN_CAP`: он не материализует список и не читает ни одного файла. Исходный дефект — материализация каталога и применение бюджета после обхода — не вернулся |
...
## Открытые находки живого стенда

Аудит `3b79b43` закрыт полностью. Эта находка получена не из аудита, а из
прогона на настоящих узлах: на малом стенде она невидима, поэтому счётчик
«ноль открытых» без неё был бы ложным.

| Finding | Измерение | Root cause (замер, не чтение кода) | Что считать исправленным |
|---|---|---|---|
| PULSE-088 (стоимость такта растёт со шкалой узла) | `pulse scorecard --seconds 12` и `strace -qq -f -c pulse check` на двух staging-узлах: `docker-stage-2` (467 сущностей, 11 043 серии) — такт 93…119 мс, ~30 000 syscall; `docker-stage-1` (1313 сущностей, 27 912 серий) — такт 325…355 мс, p95 391 мс, 36.45 % одного ядра при интервале 1 с, ~72 000 syscall | Разовый профиль фаз такта показал: 84 % времени — коллекторы `process` (150…156 мс) и `cgroup` (123…130 мс); `ingest`+`analyze`+`snapshot` ≈ 17 мс, то есть гипотеза «дорого хранилище при 31k серий» **опровергнута**. По `strace`: 9894 открытия `/proc` против 2921 cgroupfs, `stat` читается трижды на процесс (2700 при 899 процессах) | p95 полного такта ≤ 20 % интервала на узле масштаба `docker-stage-1` без потери сущностей, при снижении числа `open`/`read` в `strace -c` на том же узле. Профиль фаз для приёмки не нужен: syscall-счётчик и суммарный такт различают «убрали работу» и «перенесли в другую фазу» |

Числа сняты `2026-09-22` на ядре `6.8.0-94-generic`, 16 vCPU,
15.5 ГиБ RAM (`MemTotal: 16306764 kB`), при свободном `127.0.0.1:9095-9099`.
Конфигурация узлов не менялась: бинарник запускался из `/tmp` и удалён после
прогона.

**Прогресс (не закрытие).** Убран дубль обхода `/proc`: `HostCollector` читал
`stat` каждого PID ради счётчиков процессов и потоков, которые
`ProcessCollector` и так получает из уже прочитанного `stat`
(`process_counts_are_not_walked_twice`). Замер на `docker-stage-1` тем же
`strace -qq -f -c pulse check`: **72 308 → 66 454** системных вызовов (−8 %),
открытий файлов 12 974 → 11 904; такт `scorecard`: среднее 346 → 328 мс,
p95 **391 → 339 мс** (−13 %). Это 34 % интервала против целевых 20 %, поэтому
finding остаётся открытым: основная стоимость по-прежнему в `process`
(8 файлов на процесс) и `cgroup` (≈14 файлов на узел).

**Прогресс, шаг 2 (не закрытие).** Два источника лишних syscall без смены
контрактов данных:

- `FsSource::read_single` для однозаписных файлов (`/proc/<pid>/*` —
  `single_open`, файлы cgroup — `seq_show`): короткое чтение уже конец,
  завершающий `read` с нулём не нужен. Многозаписные файлы
  (`/proc/net/tcp`, `/proc/diskstats`) им не читаются: seq_file может вернуть
  короткое чтение до конца данных
  (`real_single_read_continues_past_full_chunk_and_obeys_cap`).
- `PROC_SHARED` берётся из уже прочитанного `status` (`RssFile + RssShmem`)
  вместо отдельного `statm` на каждый процесс
  (`shared_memory_comes_from_status_without_reading_statm`). Сверка на
  `docker-stage-1` скриптом по всем PID: 703 совпадения, 0 расхождений,
  238 процессов без `mm` — у всех `statm` = 0, как и новый путь.

A/B на `docker-stage-1` `2026-09-23`: плечо A `9b9494e`, плечо B `a225235`,
одинаковый конфиг, чередование A/B/A/B. `strace -qq -f -c pulse check`:
**68 959 → 54 902** syscall (−20 %), `read` 22 437 → 11 122, `close`
(открытые файлы) 12 483 → 11 516. `pulse scorecard --seconds 20`: среднее
такта 351 / 341 → 327 / 329 мс (−5…7 %), p95 414 / 398 → 363 / 429 мс —
на 20 тактах p95 это одна выборка, разброс больше эффекта. Время упало
меньше, чем число вызовов: пустой `read` дёшев, стоимость в `open` и в
генерации содержимого ядром, а число файлов задано контрактами.

**Прогресс, шаг 3 (не закрытие), 2026-09-26.** Исследование и `perf` на
`docker-stage-1` показали, что lossless-узкое место шире числа `read`: узел
имеет **240 possible CPU при 16 online** (`/sys/devices/system/cpu/possible`
и `online`). Ядро агрегирует PSI/rstat по possible CPU; три `*.pressure` на
222 cgroup заняли 51 из 77 мс чистой стоимости чтений cgroup в отдельном
микрозамере. `perf record -g -F 999` подтвердил 71,95 % семплов в ядре:
`psi_show` 11,47 %, `cgroup_seqfile_show` 18,23 %, `path_openat` 12,02 %.
На `docker-stage-2` те же 240 possible / 16 online, но cgroup и процессов
меньше, поэтому finding там не проявляется.

Приняты только изменения без потери свежести:

- число fd берётся из `st_size` `/proc/<pid>/fd` на procfs Linux 6.2+
  (интерфейс ядра для мониторинга), с точным прежним обходом на старом ядре
  и вне procfs; live-сверка дала 936 совпадений, два расхождения были
  изменением fd между последовательными командами;
- `status`, `io`, `cpu.stat`, `memory.stat`, `memory.events` разбираются одним
  проходом по всем нужным ключам;
- UTF-8-строки больше не копируются второй раз; множество живых PID больше не
  проверяется квадратичным `Vec::contains`.

A/B одинаковым конфигом `[export] enabled=false`,
чередование A/B/A/B по 20 тактов: среднее **334,5 / 347,7 → 288,7 /
283,6 мс** (−14…18 %), p95 **401,7 / 373,6 → 329,1 / 321,6 мс**
(−14…18 %); CPU `user` 2,04 / 1,92 → 1,52 / 1,46 с, `sys` 5,00 / 5,02 →
4,49 / 4,60 с. `getdents64` одного `check`: 2705 → 1164; рост
`newfstatat` 1439 → 2339 — ожидаемая замена дорогого обхода fd-каталогов
одним точным stat каждого каталога.

Удержание fd с `pread(0)` отвергнуто: оно быстрее (`/proc` 63,2 → 29,4 мс,
cgroup 85,7 → 62,6 мс), но живой замер `memory.current` показал
≈4,6 КиБ kernel slab на каждый прочитанный seq-file fd — **33 МиБ** на
текущем узле, больше RSS Pulse и скрыто от RSS-метрики. Группировка чтений
по имени cgroup-файла также отвергнута замером: 85,7 → 100,0 мс.

Finding остаётся открытым: lossless-путь заметно улучшил стоимость, но p95
всё ещё выше 20 % интервала. Дальнейшее крупное снижение требует продуктового
выбора: реже читать per-cgroup PSI (до 2 с задержки; ядро обновляет averages
с `PSI_FREQ = 2*HZ+1`) либо поднять допустимый интервал на узлах с большим
`possible CPU`. Кэш `/proc/<pid>/cgroup`, пропуск `oom_score`/`io` и кэш
лимитов cgroup не принимаются: они меняют наблюдаемые данные.
