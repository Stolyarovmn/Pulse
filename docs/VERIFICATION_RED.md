...
|---|---|---|---|
| PULSE-088 (стоимость такта растёт со шкалой узла) | `PULSE_PROFILE=1 pulse scorecard --seconds 12` на двух staging-узлах: `docker-stage-2` (467 сущностей, 11 043 серии) — total 93…119 мс; `docker-stage-1` (1313 сущностей, 27 912 серий) — total 325…355 мс, p95 391 мс, 36.45 % одного ядра при интервале 1 с | 84 % такта — два коллектора: `process` 150…156 мс и `cgroup` 123…130 мс. Остальное мало: `host` ≈ 22 мс, `ingest` 5.5…25.6 мс, `analyze` ≈ 8 мс, `snapshot` ≈ 3 мс. Гипотеза «дорого хранилище при 31k серий» замером **опровергнута** | p95 полного такта ≤ 20 % интервала на узле масштаба `docker-stage-1` без потери сущностей; профиль подтверждает снижение именно `process`/`cgroup`, а не перенос работы в другую фазу |

Сырые числа профиля сняты `2026-09-22` на ядре `6.8.0-94-generic`, 16 vCPU,
