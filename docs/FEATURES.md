...
| Демонстрационный сценарий `--demo`: подменяется только `FsSource`, конвейер и правила настоящие; цикл покой → деградация → восстановление; пометка `DEMO` в шапке каждого кадра | `pulse-collect/src/demo.rs` (`DemoFs`), `pulse-cli/src/main.rs` (`source_fs`), `pulse-cli/tests/demo_scenario.rs` | реализовано |
| Ось времени с отметками: события ставятся в свой момент, худшее состояние выигрывает колонку, таймкоды подписываются без наложения; без событий строка остаётся подписью границ наблюдения | `pulse-tui/src/screens/timeline.rs` (`rail_marks`, `rail_labels`) | реализовано |
