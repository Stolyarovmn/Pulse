...
| Главный процесс сервиса — корень дерева (`ppid` вне сервиса), запасной критерий `(start_ticks, pid)`; подпись «кандидат на главный процесс» не выдаёт эвристику за MainPID | `pulse-tui/src/pipe.rs` (`main_process`, `parent_pid`), `pulse-collect/src/process.rs` | реализовано |
...
| Системные журналы как постоянный коллектор/метки Timeline | — | не реализовано: нужен курсорный источник с отдельным memory/work budget (`docs/ROADMAP.md` 3.7) |
